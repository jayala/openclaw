...
- Voice wake: run `voice.transcript` commands with the target agent's `thinkingDefault` when it sets one, instead of always forcing `low`, so a dedicated voice agent can answer spoken commands with thinking off.
