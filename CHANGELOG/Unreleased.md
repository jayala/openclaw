...
- Agents: publish a reply claimed by a `before_agent_reply` plugin hook as an assistant delta, so chat subscribers (Control UI, nodes that speak replies) no longer receive a final chat event without message text for hook-handled turns.
